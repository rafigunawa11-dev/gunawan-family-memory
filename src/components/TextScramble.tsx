  ...
  return (
    ...
    >
      ...
      <span className="relative h-px w-full mt-2 overflow-hidden">
        <span
          className={`absolute inset-0 bg-[--color-text] transition-transform duration-500 ease-out origin-left ${
            isHovering ? "scale-x-100" : "scale-x-0"
          }`}
        />
        <span className="absolute inset-0 bg-[--color-border]" />
      </span>

      <span
        className={`absolute -inset-4 rounded-lg bg-[--color-accent]/5 transition-opacity duration-300 -z-10 ${
